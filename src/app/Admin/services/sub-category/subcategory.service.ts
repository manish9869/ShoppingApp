...
export class SubCategoryService {
  ...
  private subcategoryDataUpdated = new Subject<{
  ...
  getSubCategoryListdb() {
    this.http
      .get<{ message: string; subcategoryData: any }>(BACKEND_URL)
      .subscribe((result) => {
  ...
  UpdateCategoryStatus(Subcategoryid: string, status: boolean) {
    const subcategoryData: SubCategoryData = {
      _id: Subcategoryid,
      categoryId: null,
    ...
    return this.http.post(
      BACKEND_URL + "/updateStatus" + "/" + Subcategoryid,
      subcategoryData
    );
  }
}
